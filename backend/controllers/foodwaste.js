...
router.get('/:year/:id', checkUserBelongsToFridge, async (req, res) => {
  ...
  if (req.params.id && req.params.year) {
    ...
    const wastePerMonth = await WasteProduct.findAll({
      ...
      where: {
        [Op.and]: [
          ...
          sequelize.where(sequelize.literal('extract(YEAR FROM "wasteProduct"."date")'), req.params.year)
        ]
      },
      group: [sequelize.fn('date_trunc', 'month', sequelize.col('date')), 'month']
    ...
    if (wastePerMonth && fridgeWaste && othersWaste && numberOfOtherFridges && mostOftenExpiring) {
      ...
      if (fridgeWasteTot > othersWasteTot) {
        const percent = parseInt(((fridgeWasteTot - othersWasteTot) / othersWasteTot) * 100)
        res.status(200).json({ wastePerMonth: wastePerMonth, moreThanOthers: true, MTOPercent: percent, mostOftenExpiring: fourMostOftenExpiring })
      } else {
        const percent = parseInt(((othersWasteTot - fridgeWasteTot) / othersWasteTot) * 100)
        res.status(200).json({ wastePerMonth: wastePerMonth, moreThanOthers: false, MTOPercent: percent, mostOftenExpiring: fourMostOftenExpiring })
      }
